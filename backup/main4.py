...
from groq_ai import (
    ...
    route_recommendation_request_groq,
    emotions_top3_to_profile
)
    ...
    try:
        ...
        if "get_recommendations" in intent:
            ...
            try:
                ...
                # Analyze / Route (LLM Router: artist vs mood vs general)
                emotional_profile = {}
                route_info = {"route": "general", "confidence": 0.0}

                normalized_msg = (user_message or '').strip()

                # "Truly generic" = very short + no clear constraints
                generic_phrases = ["แนะนำเพลง", "เพลงแนะนำ", "ขอเพลงหน่อย", "หาเพลง", "recommend songs", "recommend song"]
                cleaned = re.sub(r"\s+", " ", normalized_msg.lower()).strip()
                is_truly_generic = (len(cleaned) <= 10) and any(p in cleaned for p in generic_phrases)

                if is_truly_generic:
                    logging.info("Generic request detected (truly generic). Using PURE User Taste Profile.")
                else:
                    # Always ask router for flexible requests (e.g., driving chill, study, rainy day, sad songs)
                    logging.info("Routing request via LLM router (top-3 emotions)...")
                    try:
                        route_info = await route_recommendation_request_groq(normalized_msg)
                    except Exception as e:
                        logging.warning(f"Router failed, fallback to general: {e}")
                        route_info = {"route": "general", "confidence": 0.0}

                    # Confidence gate: avoid overfitting to weak router guesses
                    conf = float(route_info.get("confidence", 0.0) or 0.0)
                    route = str(route_info.get("route", "general") or "general").strip().casefold()

                    logging.info(f"🧭 Route selected: {route} | confidence={conf:.2f} | artist_mode={route_info.get('artist_mode')} | artist_name={route_info.get('artist_name')}")
                    if route_info.get("emotions"):
                        logging.info(f"🎭 Intent Mood (top3): {route_info.get('emotions')}")
                        emotional_profile = emotions_top3_to_profile(route_info.get("emotions", []))
                    else:
                        logging.info("🎭 Intent Mood (top3): (none)")

                    if conf < 0.55 and route != "artist":
                        logging.info("Router confidence low. Falling back to PURE User Taste Profile.")
                        route_info = {"route": "general", "confidence": conf}
                        emotional_profile = {}

                # Log user taste top weights (debug)
                try:
                    top_user = sorted([(k, float(v or 0.0)) for k, v in (historical_profile or {}).items()], key=lambda x: x[1], reverse=True)[:5]
                    logging.info(f"🧠 User Mood/Taste (top): {top_user}")
                except Exception:
                    pass
